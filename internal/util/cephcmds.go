...
func GetPoolID(monitors string, cr *Credentials, poolName string) (int64, error) {
	...
	if errors.Is(err, rados.ErrNotFound) {
		return InvalidPoolID, ErrPoolNotFound.Wrap(fmt.Errorf("pool (%s) not found in Ceph cluster", poolName))
	} else if err != nil {
		return InvalidPoolID, err
	}
...
func GetPoolName(monitors string, cr *Credentials, poolID int64) (string, error) {
	...
	if err != nil {
		return "", ErrPoolNotFound.Wrap(fmt.Errorf("pool ID (%d) not found in Ceph cluster", poolID))
	}
	return name, nil
}
...
func CreateObject(ctx context.Context, monitors string, cr *Credentials, poolName, namespace, objectName string) error {
	...
	if err != nil {
		if ErrPoolNotFound.In(err) {
			err = ErrObjectNotFound.Wrap(err)
		}
		return err
	}
	defer ioctx.Destroy()
	...
	if errors.Is(err, rados.ErrObjectExists) {
		return ErrObjectExists.Wrap(err)
	} else if err != nil {
